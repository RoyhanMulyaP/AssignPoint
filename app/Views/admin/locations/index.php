    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-6 gradient-bg">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-3xl font-bold text-white">Locations</h2>
                <button onclick="document.getElementById('addLocationModal').classList.remove('hidden')" class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-xl font-bold transition-all shadow-lg hover:shadow-red-600/20">
                ...
                <?php foreach ($locations as $loc): ?>
                    <div class="bg-gray-800/70 border border-gray-700 rounded-2xl p-6 hover:border-red-500/50 transition-all group">
                        <div class="flex justify-between items-start mb-4">
                            <div class="w-12 h-12 bg-blue-900/30 rounded-xl flex items-center justify-center text-blue-400 group-hover:scale-110 transition-transform">
                                <i class="fas fa-map-marker-alt text-xl"></i>
                            </div>
                            <form action="<?= base_url('admin/locations/delete/' . $loc['id']) ?>" method="POST">
                        ...
                        <p class="text-gray-400 text-sm"><?= $loc['description'] ?></p>
                    </div>
                <?php endforeach; ?>
            </div>
        </main>
...
<!-- Add Modal -->
<div id="addLocationModal" class="hidden fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div class="bg-gray-800 border border-gray-700 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
        <div class="p-6 border-b border-gray-700 flex justify-between items-center">
            <h3 class="text-xl font-bold text-white">Add New Location</h3>
            <button onclick="document.getElementById('addLocationModal').classList.add('hidden')" class="text-gray-400 hover:text-white">
                <i class="fas fa-times"></i>
            </button>
        </div>
        <form action="<?= base_url('admin/locations/save') ?>" method="POST" class="p-6 space-y-4">
            <div>
                <label class="block text-gray-400 text-sm mb-2">Location Name</label>
