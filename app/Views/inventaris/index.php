...
<div class="min-h-screen bg-gray-900">
    ...
    <div class="container mx-auto px-4 py-6">
            ...
            <div class="bg-gray-800 rounded-xl p-6 border border-gray-700">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <h3 class="text-3xl font-bold text-white">1,206</h3>
                        <p class="text-green-400 text-sm mt-2">
                            <i class="fas fa-check-circle mr-1"></i>
                            96.6% dari total stok
                        </p>
                    </div>
                        ...
                        <tr>
                            <th class="py-4 px-6 text-left text-gray-400 font-semibold uppercase text-sm">
                                <div class="flex items-center">
                                    <input type="checkbox" id="selectAll" class="mr-3 rounded border-gray-600 bg-gray-700 text-red-600 focus:ring-red-600">
                                    Kode Barang
                                </div>
                            </th>
                            ...
                            <tr>
                                <td class="py-4 px-6 items-center">
                                    <input type="checkbox" name="selected[]"
                                        value="<?= $item['id'] ?>" class="mr-3 text-blue-600 bg-gray-800 border-gray-600 rounded focus:ring-blue-500">
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex space-x-2">
                                        ...
                                        <button onclick="showEditModal(this)" class="bg-blue-900/30 w-8 h-8 rounded-lg">
        ...
        <!-- Bulk Actions -->
        <div id="bulkActions" class="hidden bg-gray-800 rounded-xl p-4 mt-6 border border-gray-700">
            <div class="flex justify-between items-center">
                <div class="text-white">
                    <span id="selectedCount">0</span> barang terpilih
                </div>
                <div class="flex space-x-3">
                    <button onclick="bulkExport()" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors duration-300 flex items-center">
                        <i class="fas fa-download mr-2"></i>
                        Export
                    </button>
                    <button onclick="bulkDelete()" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium transition-colors duration-300 flex items-center">
...
<script>
    ...
    // Tampilkan modal edit dan isi data dari tabel
    function showEditModal(button) {
        const row = button.closest('tr');

        const id = row.querySelector('input[name="selected[]"]').value;
    ...
    function bulkExport() {
        const selectedItems = Array.from(document.querySelectorAll('tbody input[type="checkbox"]:checked'))
            .map(checkbox => checkbox.closest('tr').querySelector('.font-semibold').textContent);

        console.log('Exporting items:', selectedItems);
        showToast(`Mengekspor ${selectedItems.length} barang`, 'success');
    }
    ...
    function bulkDelete() {
        ...
        if (selectedCount === 0) {
            showToast('Tidak ada barang yang dipilih', 'error');
            return;
        }

        // Ambil nama barang yang dipilih
        const names = Array.from(checkboxes).map(cb => cb.closest('tr').querySelector('td:nth-child(2)').textContent);
        const nameList = names.join(', ');

        // Tampilkan modal
        document.getElementById('deleteItemName').textContent = `Apakah Anda yakin ingin menghapus ${selectedCount} barang: ${nameList}?`;
        document.getElementById('deleteModal').classList.remove('hidden');
        ...
        newBtn.addEventListener('click', async function() {
            try {
                ...
                const response = await fetch(`<?= base_url('admin/inventaris/bulkDelete') ?>`, {
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                ...
                if (data.status === 'success') {
                    // Hapus row dengan animasi fade out
                    ids.forEach(id => {
                        const row = document.querySelector(`tbody input[value="${id}"]`)?.closest('tr');
                        if (row) {
                            row.style.transition = 'opacity 0.5s ease';
                            row.style.opacity = 0;
                            setTimeout(() => {
                                ...
                                // Update total, start & end setelah row dihapus
                                const totalElement = document.getElementById('totalBarang');
                                if (totalElement) totalElement.textContent = data.total;
                    ...
                    // Reset select all & sembunyikan bulkActions
                    document.getElementById('selectAll').checked = false;
                    document.getElementById('bulkActions').classList.add('hidden');

                    closeDeleteModal();
                    showToast(`${selectedCount} barang berhasil dihapus`, 'success');
                } else {
                    showToast(data.message, 'error');
                }
    ...
    document.getElementById('editForm').addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (data.status === 'success') {
                showToast(data.message, 'success');
                closeEditModal();
                location.reload(); // refresh tabel agar data update
            } else {
                showToast('Gagal mengupdate barang', 'error');
            }
        } catch (err) {
